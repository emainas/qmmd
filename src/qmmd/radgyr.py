...
def load_config(yaml_path: Path) -> RadgyrConfig:
    ...
    radgyr = data.get("radgyr", {})
    if not radgyr:
        raise RuntimeError("radgyr block is required")
    mask = radgyr.get("mask")
    if not mask:
        raise RuntimeError("radgyr.mask is required")
    name = radgyr.get("name", "radgyr")
...
def system_base_dir(cfg: RadgyrConfig, repo_root: Path) -> Path:
    return repo_root / "systems" / cfg.system / f"{cfg.prefix}_{cfg.buffer:.1f}"


def run_dir(cfg: RadgyrConfig, repo_root: Path, run_id: int) -> Path:
...
def resolve_parm_path(cfg: RadgyrConfig, repo_root: Path) -> Path:
...
def write_cpptraj_in(cfg: RadgyrConfig, parm: Path, traj: Path, out_dir: Path) -> Path:
    out_dat = out_dir / f"radgyr_{cfg.name}.dat"
    parts = [
        f"parm {parm}",
        f"trajin {traj}",
        f"radgyr {cfg.name} {cfg.mask} out {out_dat.name}",
        "run",
        "",
    ]
...
def run_radgyr(yaml_path: Path) -> None:
    ...
    for run_id in cfg.run_ids:
        ...
        run_cpptraj(cppin)
        print(f"OK: wrote {out_dir / f'radgyr_{cfg.name}.dat'}")


def main() -> None:
    if len(sys.argv) != 2:
        print("Usage: python3 radgyr.py configs/SYS/analysis/radgyr.yaml", file=sys.stderr)
        raise SystemExit(2)
    run_radgyr(Path(sys.argv[1]).resolve())
