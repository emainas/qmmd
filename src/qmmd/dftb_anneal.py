...
def load_config(yaml_path: Path) -> SimulationConfig:
    ...
    anneal_lines = data.get("anneal")
    if not anneal_lines:
        raise RuntimeError("anneal must be provided (list of lines for anneal.dat)")
    if not isinstance(anneal_lines, list) or not all(isinstance(x, str) for x in anneal_lines):
        raise RuntimeError("anneal must be a list of strings (lines for anneal.dat)")
...
def write_anneal_dat(out_dir_path: Path, anneal_lines: List[str]) -> Path:
    anneal_path = out_dir_path / "anneal.dat"
    anneal_path.write_text("\n".join(anneal_lines).rstrip() + "\n")
    return anneal_path
...
def run_dftb_anneal_submit(yaml_path: Path) -> None:
    cfg = load_config(yaml_path)
    repo_root = find_repo_root(yaml_path)
    bench_dir = out_dir(cfg, repo_root)
    yaml_text = yaml_path.read_text()
...
def main() -> None:
    if len(sys.argv) != 2:
        print("Usage: python3 dftb_anneal.py configs/SYS/dftb/dftb.yaml", file=sys.stderr)
        raise SystemExit(2)

    run_dftb_anneal_prep(Path(sys.argv[1]).resolve())
