...
def system_base_dir(cfg: WTMetaConfig, repo_root: Path) -> Path:
    return repo_root / "systems" / cfg.system / f"{cfg.prefix}_{cfg.buffer:.1f}"


def dftb_root_dir(cfg: WTMetaConfig, repo_root: Path) -> Path:
    return system_base_dir(cfg, repo_root) / cfg.dftb_dirname


def run_dir(cfg: WTMetaConfig, repo_root: Path, run_id: int) -> Path:
...
def repo_params_dir(cfg: WTMetaConfig, repo_root: Path) -> Path:
...
def load_config(yaml_path: Path) -> WTMetaConfig:
...
def parse_bathtemp(header_lines: List[str]) -> float:
    for line in header_lines:
        if "BATHTEMP=" not in line:
            continue
        parts = line.replace(",", " ").replace(")", " ").replace("(", " ").split()
        for part in parts:
            if part.startswith("BATHTEMP="):
                try:
                    return float(part.split("=", 1)[1])
                except ValueError:
                    continue
    raise RuntimeError("Could not find BATHTEMP= in dftb.header_lines")


def apply_wtmeta_header_lines(header_lines: List[str], gamma: float) -> List[str]:
    bathtemp = parse_bathtemp(header_lines)
    bias_temp = (gamma - 1.0) * bathtemp
    bias_line = f"MD=(WTMETABIASTEMP={bias_temp:g})"

    updated: List[str] = []
    bias_replaced = False
    has_wtmeta = False
    for line in header_lines:
        if "MD=(WTMETADYNAMICS=" in line:
            has_wtmeta = True
        if "MD=(WTMETABIASTEMP=" in line:
            updated.append(bias_line)
            bias_replaced = True
            continue
        updated.append(line)

    if not has_wtmeta:
        print("WARN: Missing MD=(WTMETADYNAMICS=...) in dftb.header_lines; using as-is")
    if not bias_replaced:
        updated.append(bias_line)
    return updated
...
def write_dftb_inp(
    ...
    header_lines = apply_wtmeta_header_lines(header_lines, cfg.gamma)
...
def write_run_sh(cfg: WTMetaConfig, out_dir_path: Path) -> Path:
...
def write_slurm_sh(cfg: WTMetaConfig, out_dir_path: Path) -> Optional[Path]:
...
def run_wtmeta_prep(yaml_path: Path) -> None:
    ...
    for run_id in cfg.run_ids:
        ...
        (cv_dir / "wtmeta_spec.yaml").write_text(yaml_text)

        print(f"OK: wrote wtmeta inputs in {cv_dir} (run {run_id})")


def run_wtmeta_submit(yaml_path: Path) -> None:
    cfg = load_config(yaml_path)
    repo_root = find_repo_root(yaml_path)
    yaml_text = yaml_path.read_text()
    ...
    for run_id in cfg.run_ids:
        rdir = run_dir(cfg, repo_root, run_id)
        cv_dir = rdir / cfg.cv_dirname
        spec = cv_dir / "wtmeta_spec.yaml"
        if not spec.exists() or spec.read_text() != yaml_text:
            print(f"SKIP: {cv_dir} wtmeta_spec.yaml does not match config (not submitting)")
            continue
    ...
    if not targets:
        print("NOTE: no matching wtmeta dirs found; nothing submitted")
        return

    print("Will submit the following wtmeta dirs:")
    for t in targets:
        print(f"  - {t}")
...
def main() -> None:
    if len(sys.argv) != 2:
        print("Usage: python3 wtmeta.py configs/SYS/wtmeta/wtmeta.yaml", file=sys.stderr)
        raise SystemExit(2)
    run_wtmeta_prep(Path(sys.argv[1]).resolve())
