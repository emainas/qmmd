...
def load_config(yaml_path: Path) -> DihedralConfig:
    ...
    dihs = data.get("dihedrals")
    if not dihs:
        raise RuntimeError("dihedrals list is required")
    dihedral_specs: List[DihedralSpec] = []
    for d in dihs:
        if "atoms" not in d:
            raise RuntimeError("Each dihedral must include 'atoms' (list of 4 masks)")
        atoms = [str(x) for x in d["atoms"]]
        if len(atoms) != 4:
            raise RuntimeError(f"Dihedral atoms must have 4 entries, got {atoms}")
        indices = d.get("indices")
        if indices is not None:
            indices = [int(x) for x in indices]
            if len(indices) != 4:
                raise RuntimeError(f"Dihedral indices must have 4 entries, got {indices}")
...
def system_base_dir(cfg: DihedralConfig, repo_root: Path) -> Path:
    return repo_root / "systems" / cfg.system / f"{cfg.prefix}_{cfg.buffer:.1f}"


def run_dir(cfg: DihedralConfig, repo_root: Path, run_id: int) -> Path:
...
def resolve_parm_path(cfg: DihedralConfig, repo_root: Path) -> Path:
...
def _format_dih_filename(spec: DihedralSpec) -> str:
    if spec.indices:
        return f"dih_{spec.indices[0]}_{spec.indices[1]}_{spec.indices[2]}_{spec.indices[3]}.dat"
    if spec.name:
        return f"dih_{_sanitize(spec.name)}.dat"
    atom_tag = "_".join(_sanitize(a) for a in spec.atoms)
    return f"dih_{atom_tag}.dat"
...
def write_cpptraj_in(cfg: DihedralConfig, parm: Path, traj: Path, out_dir: Path) -> Path:
    parts: List[str] = [
        f"parm {parm}",
        _trajin_line(traj),
    ]
...
def run_dihedral(yaml_path: Path) -> None:
    ...
    for run_id in cfg.run_ids:
        ...
        run_cpptraj(cppin)
        for spec in cfg.dihedrals:
            print(f"OK: wrote {out_dir / _format_dih_filename(spec)}")


def main() -> None:
    if len(sys.argv) != 2:
        print("Usage: python3 dihedral.py configs/SYS/analysis/dihedral.yaml", file=sys.stderr)
        raise SystemExit(2)
    run_dihedral(Path(sys.argv[1]).resolve())
