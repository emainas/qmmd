...
def compute_run(cfg: CVCfg, run: CVRun) -> Path:
    ...
    if cfg.equil_dirname and cfg.equil_traj_name:
        ...
        for _, coords in iterator:
            ...
            c1 = coords[g1].mean(axis=0)
            c2 = coords[g2].mean(axis=0)
            rows.append(float(np.linalg.norm(c1 - c2)))
...
def run_cv_dist(yaml_path: Path) -> None:
...
def main() -> None:
    if len(sys.argv) != 2:
        print("Usage: python3 cv_dist.py configs/SYS/cv/cv_dist.yaml", file=sys.stderr)
        raise SystemExit(2)
    run_cv_dist(Path(sys.argv[1]).resolve())
