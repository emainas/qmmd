...
def load_config(yaml_path: Path) -> HBondConfig:
    ...
    hbonds_data = data.get("hbonds")
    if not hbonds_data:
        raise RuntimeError("hbonds list is required")
    hbonds: List[HBondSpec] = []
    for h in hbonds_data:
        name = h.get("name")
        if not name:
            raise RuntimeError("Each hbond must include 'name'")
        donor = h.get("donormask")
        acceptor = h.get("acceptormask")
        if not donor or not acceptor:
            raise RuntimeError("Each hbond must include donormask and acceptormask")
        hbonds.append(
            HBondSpec(
                name=str(name),
                donormask=str(donor),
                acceptormask=str(acceptor),
                distance=h.get("distance"),
                angle=h.get("angle"),
                noimage=bool(h.get("noimage", False)),
                series=h.get("series"),
            )
        )

    lifetime_cfg = data.get("lifetime", {})
    lifetime = LifetimeConfig(
        enabled=bool(lifetime_cfg.get("enabled", False)),
        window=lifetime_cfg.get("window"),
    )
...
def system_base_dir(cfg: HBondConfig, repo_root: Path) -> Path:
    return repo_root / "systems" / cfg.system / f"{cfg.prefix}_{cfg.buffer:.1f}"


def run_dir(cfg: HBondConfig, repo_root: Path, run_id: int) -> Path:
...
def resolve_parm_path(cfg: HBondConfig, repo_root: Path) -> Path:
...
def convert_dftb_xyz_to_cpptraj(src: Path, dst: Path, dftb_inp: Path) -> None:
...
def write_cpptraj_in(cfg: HBondConfig, parm: Path, traj: Path, out_dir: Path) -> Path:
    parts: List[str] = [
        f"parm {parm}",
        f"trajin {traj}",
    ]

    for spec in cfg.hbonds:
        out_dat = out_dir / f"hbond_{spec.name}.dat"
        avg_dat = out_dir / f"hbond_{spec.name}.avg"
        series_name = f"hbond_{spec.name}"
        cmd = [
            "hbond",
            spec.name,
            "donormask",
        ...
        if spec.distance is not None:
            cmd.extend(["distance", str(spec.distance)])
        if spec.angle is not None:
            cmd.extend(["angle", str(spec.angle)])
        if spec.noimage:
            cmd.append("noimage")
        use_series = spec.series if spec.series is not None else cfg.lifetime.enabled
        if use_series:
            cmd.extend(["series", "uuseries", f"{series_name}.dat"])
    ...
    if cfg.lifetime.enabled:
        for spec in cfg.hbonds:
            series_name = f"hbond_{spec.name}"
            life_dat = out_dir / f"hbond_{spec.name}.life.dat"
            life_cmd = f"runanalysis lifetime HB[{series_name}] out {life_dat.name}"
            if cfg.lifetime.window is not None:
                life_cmd += f" window {cfg.lifetime.window}"
            parts.append(life_cmd)
...
def run_hbond(yaml_path: Path) -> None:
    ...
    for run_id in cfg.run_ids:
        ...
        if not traj.exists() or traj.stat().st_size == 0:
            raise RuntimeError(f"Missing/empty traj: {traj}")
        ...
        run_cpptraj(cppin)
        for spec in cfg.hbonds:
            print(f"OK: wrote {out_dir / f'hbond_{spec.name}.dat'}")
            print(f"OK: wrote {out_dir / f'hbond_{spec.name}.avg'}")
            if cfg.lifetime.enabled:
                print(f"OK: wrote {out_dir / f'hbond_{spec.name}.life.dat'}")


def main() -> None:
    if len(sys.argv) != 2:
        print("Usage: python3 hbond.py configs/SYS/analysis/hbond.yaml", file=sys.stderr)
        raise SystemExit(2)
    run_hbond(Path(sys.argv[1]).resolve())
