...
def load_config(yaml_path: Path) -> RMSDConfig:
    ...
    rmsd = data.get("rmsd", {})
    if not rmsd:
        raise RuntimeError("rmsd block is required")
    mask = rmsd.get("mask")
    if not mask:
        raise RuntimeError("rmsd.mask is required")
    name = rmsd.get("name", "rmsd")
...
def system_base_dir(cfg: RMSDConfig, repo_root: Path) -> Path:
    return repo_root / "systems" / cfg.system / f"{cfg.prefix}_{cfg.buffer:.1f}"


def run_dir(cfg: RMSDConfig, repo_root: Path, run_id: int) -> Path:
...
def resolve_parm_path(cfg: RMSDConfig, repo_root: Path) -> Path:
...
def write_cpptraj_in(cfg: RMSDConfig, parm: Path, traj: Path, out_dir: Path) -> Path:
    out_dat = out_dir / f"rmsd_{cfg.name}.dat"
    parts = [
        f"parm {parm}",
        f"trajin {traj}",
        f"rmsd {cfg.name} {cfg.mask} out {out_dat.name}",
        "run",
        "",
    ]
...
def run_rmsd(yaml_path: Path) -> None:
    ...
    for run_id in cfg.run_ids:
        ...
        run_cpptraj(cppin)
        print(f"OK: wrote {out_dir / f'rmsd_{cfg.name}.dat'}")


def main() -> None:
    if len(sys.argv) != 2:
        print("Usage: python3 rmsd.py configs/SYS/analysis/rmsd.yaml", file=sys.stderr)
        raise SystemExit(2)
    run_rmsd(Path(sys.argv[1]).resolve())
