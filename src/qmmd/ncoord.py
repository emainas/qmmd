...
def dftb_root_dir(cfg: NcoordConfig, repo_root: Path) -> Path:
    return system_base_dir(cfg, repo_root) / cfg.dftb_dirname


def run_dir(cfg: NcoordConfig, repo_root: Path, run_id: int) -> Path:
...
def select_group_indices(group: GroupSpec, symbols: List[str]) -> List[int]:
    ...
    if mode in ("all_water_h", "all_h"):
        ...
        merged: List[int] = []
        seen = set()
        # preserve requested order: extras first, then all_H
...
def write_metacv(path: Path, cfg: NcoordConfig, group1: List[int], group2: List[int]) -> None:
    header = (
        f"{cfg.cv_title} {cfg.gausswidth} {len(group1)} {len(group2)} "
        f"{cfg.nexp} {cfg.mexp} {cfg.refdist} {cfg.method} {cfg.norm} "
...
def run_ncoord(yaml_path: Path) -> None:
    ...
    for run_id in cfg.run_ids:
        ...
        group1 = select_group_indices(cfg.group1, symbols)
        group2 = select_group_indices(cfg.group2, symbols)

        metacv = cv_dir / "metacv.dat"
        write_metacv(metacv, cfg, group1, group2)
...
def main() -> None:
    if len(sys.argv) != 2:
        print("Usage: python3 ncoord.py configs/SYS/ncoord/ncoord.yaml", file=sys.stderr)
        raise SystemExit(2)
