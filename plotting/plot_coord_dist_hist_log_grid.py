#!/usr/bin/env python3
"""Plot 2D histogram grids of coord vs dist using -log10(count)."""
...
def main() -> None:
    p = argparse.ArgumentParser(description="Plot coord vs dist 2D histogram grids using -log10(count).")
    ...
    for i, (run_id, x, y) in enumerate(series):
        ...
        z = np.ma.masked_array(-np.log10(h), mask=h.mask)
        ax.imshow(
            z.T,
            origin="lower",
            aspect="auto",
    ...
    out = args.out
    if out is None:
        out = Path("reports") / f"{system}_{args.cv_dir}_coord_dist_hist_log_{len(runs)}runs.png"
