#!/usr/bin/env python3
"""PRD pyridinium adaptation of acid_base_HIST.py. All solute heavy atoms are competitors.
...
def plot_wire_csv(
    ...
    column_count = (
        3 + int(has_hist_column) + int(has_hist_competitor_column)
    )
    ...
    if has_hist_column:
        ...
        from prd_competitor_analysis import classify_histidine_conformers
    ...
    if has_hist_competitor_column:
        ...
        ax_competitor_charge.set_ylabel("PRD heavy-atom charge tier")
        ...
        ax_competitor_wire.set_ylabel("PRD heavy-atom site")
        ...
        ax_competitor_coord.set_yticks(
            [
                index * coordination_band_height + 1.0
                for index in range(len(competitor_labels))
            ],
            labels=[
                f"{label} ({hist_competitor_endpoint_ids[label]})"
        ...
        ax_competitor_coord.set_ylabel("PRD heavy-atom site")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=3,
        help=(
            "Maximum water bridges searched for PRD heavy-atom site paths "
    ...
    from prd_competitor_analysis import (
        calculate_histidine_competitor_geometry, save_histidine_competitor_data,
    )
    symbols = read_xyz_symbols(args.traj)
    hist_competitor_endpoints = {f"{symbol}{i}": i for i,symbol in enumerate(symbols[:args.solute_atoms],1) if symbol != "H"}
    if tuple(hist_competitor_endpoints) != ("N1","C2","C3","C4","C5","C6"):
        raise ValueError("Expected existing PRD pyridinium atom order N1,C2–C6")
    ...
    hist_competitor_wires, hist_competitor_coordination = calculate_histidine_competitor_geometry(
        ...
        angle_cutoff=args.angle_cutoff)
    hist_competitor_out = args.out.with_name(f"{args.out.stem}_prd_competitors.csv")
    ...
    if mulliken_path.exists():
        ...
        if (
            ...
            hist_charge_out = (
                ...
                or args.out.with_name(f"{args.out.stem}_prd_heavy_charges.csv")
            )
            save_histidine_charge_data(
                hist_charge_out,
                hist_charge_times,
