...
def plot_wire_csv(
    ...
    has_hist_competitor_column = (
        has_hist_column
        and hist_charge_times is not None
    ...
    column_count = (
        5 if has_hist_competitor_column else (4 if has_hist_column else 3)
    )
    ...
    if has_hist_competitor_column:
        competitor_labels = ("N", "ND1", "NE2", "O", "O1")
        competitor_colors = {
            "N": "#0072B2",
            "ND1": "#E69F00",
            "NE2": "#009E73",
        ...
        ax_competitor_charge.set_ylabel("histidine N/O charge tier")
        ...
        ax_competitor_wire.set_ylabel("histidine N/O site")
        ...
        ax_competitor_coord.set_ylabel("histidine N/O site")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=3,
        help=(
            "Maximum water bridges searched for histidine N/O-site wires "
    ...
    if args.hist_parm is not None:
        ...
        from hist_competitor_analysis import (
            calculate_histidine_competitor_geometry,
            calculate_histidine_dihedrals,
            histidine_definitions,
            save_histidine_competitor_data,
            save_histidine_dihedral_data,
        )

        hist_quartets, hist_competitor_endpoints = histidine_definitions(
            args.hist_parm, args.solute_atoms
        )
        hist_dihedrals = calculate_histidine_dihedrals(
            args.traj,
            hist_dihedral_times,
            hist_quartets,
            box,
        )
        hist_dihedral_out = args.hist_dihedral_data_out or args.out.with_name(
            f"{args.out.stem}_hist_dihedrals.csv"
        )
        save_histidine_dihedral_data(
            hist_dihedral_out,
            hist_dihedral_times,
        ...
        hist_competitor_wires, hist_competitor_coordination = (
            calculate_histidine_competitor_geometry(
                ...
                args.solute_atoms,
                box,
                max_bridging_waters=args.hist_competitor_max_bridging_waters,
                covalent_cutoff=args.covalent_cutoff,
                hydrogen_acceptor_cutoff=args.hydrogen_acceptor_cutoff,
                angle_cutoff=args.angle_cutoff,
            )
        )
        hist_competitor_out = args.hist_competitor_data_out or args.out.with_name(
            f"{args.out.stem}_hist_competitors.csv"
        )
        save_histidine_competitor_data(
            hist_competitor_out,
            hist_dihedral_times,
            aligned_defect_ids,
            hist_competitor_wires,
            hist_competitor_coordination,
        )
    ...
    if mulliken_path.exists():
        ...
        if (
            args.hist_parm is not None
            and hist_competitor_endpoints is not None
            ...
            hist_charge_out = (
                ...
                or args.out.with_name(f"{args.out.stem}_hist_N_O_charges.csv")
            )
            save_histidine_charge_data(
                hist_charge_out,
                hist_charge_times,
