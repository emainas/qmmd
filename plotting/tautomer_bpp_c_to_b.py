"""Replot BPP's saved summary as FES/CV/estimator plus NC/NB charge/LCOD/coordination.
...
def refresh_full_run(p: Path, provenance: dict, nc: int, nb: int) -> float:
    ...
    for t, xyz in xyz_frames(source/'traject'):
        ...
        coord.append([t, *[rational_coordination(np.linalg.norm(minimum_image(xyz[hs]-xyz[i-1],box),axis=1)) for i in (nc,nb)]])
    ...
    np.savetxt(p/'summary_bpp_competitors.csv',coord,delimiter=',',header='time_ps,NC_all_H_coordination,NB_all_H_coordination',comments='')
    water_ids = [i+1 for i,s in enumerate(symbols) if i>=77 and s=='O']
    endpoints = {'NC':nc,'NB':nb,**{f'O{i}':i for i in water_ids}}
    qt, q = calculate_histidine_charges(source/'mulliken',endpoints,77,stop)
    lookup = {round(float(t),8):i for i,t in enumerate(qt)}
    charge_rows = [[t,*[q[label][lookup[round(float(t),8)]] if round(float(t),8) in lookup else np.nan for label in ('NC','NB')]] for t in times]
    np.savetxt(p/'summary_bpp_heavy_charges.csv',charge_rows,delimiter=',',header='time_ps,NC_charge,NB_charge',comments='')
...
def lcod_series(source: Path, ids: tuple[int, int, int], stop: float) -> np.ndarray:
    """Return time, NC-H, NB-H, and their difference (ps and angstrom)."""
    box = box_from_input(source/'dftb.inp')
    ...
    with (source/'traject').open() as handle:
        while line := handle.readline():
            ...
            if [fields[i][0] for i in indices] != ['N', 'N', 'H']:
                raise ValueError('Expected NC, NB, H atom elements')
...
def main() -> None:
    ...
    parser.add_argument('--report', type=Path, default=Path('reports/taut_bpp_meta_hic/run-9'))
    parser.add_argument('--nc', type=int, default=30)
    parser.add_argument('--nb', type=int, default=19)
    parser.add_argument('--hydrogen', type=int, default=31, help='Fixed, one-based hydrogen ID; no ownership switching')
    ...
    parser.add_argument('--grid-onset-ps',type=float,default=50.5,help='Marker on plot_cv_grid clock, ps')
    ...
    if min(args.nc, args.nb, args.hydrogen) < 1 or len({args.nc, args.nb, args.hydrogen}) != 3:
    ...
    for atom_id, name in ((args.nc,'NC'),(args.nb,'NB'),(args.hydrogen,'HC')):
    ...
    provenance = dict(source=str(source),grid_onset_ps=args.grid_onset_ps,
                      ...
                      competitors={'NC':args.nc,'NB':args.nb},solute_atoms=77,
    ...
    if (args.nc, args.nb) != (provenance['competitors']['NC'], provenance['competitors']['NB']):
        parser.error('NC/NB IDs must agree with the saved charge/coordination products')
    onset = provenance['raw_onset_ps']
    stop = refresh_full_run(p,provenance,args.nc,args.nb)
    summary = load_csv(p/'summary_tautomer_timeseries.csv')
    charges = load_csv(p/'summary_bpp_heavy_charges.csv')
    coord = load_csv(p/'summary_bpp_competitors.csv')
    ...
    lcod = lcod_series(Path(provenance['source']), (args.nc, args.nb, args.hydrogen), stop)
    ...
    np.savetxt(p/'summary_lcod.csv', np.column_stack([summary['time_ps'], joined]), delimiter=',',
               header=f'time_ps,r_NC{args.nc}_H{args.hydrogen}_A,r_NB{args.nb}_H{args.hydrogen}_A,LCOD_A', comments='')
    ...
    for i, (label, atom) in enumerate([('NC', args.nc), ('NB', args.nb)]):
    ...
    ax_q.set(yticks=[-.35, .75], yticklabels=[f'NC{args.nc}', f'NB{args.nb}'], ylim=(-1.05, 1.45), ylabel='Mulliken charge tiers (NB offset +1.1 e)')
    ax_lcod.plot(lcod[:, 0], lcod[:, 3], color='#6A3D9A', lw=1.2)
    ax_lcod.axhline(0, color='.5', lw=.8, ls=':')
    ax_lcod.set(ylabel=f'r(NC{args.nc},H{args.hydrogen}) − r(NB{args.nb},H{args.hydrogen}) (Å)', title='Fixed-H LCOD: negative NC-side / positive NB-side')
    ...
    fig.suptitle(f'BPP {p.name}: tautomerization summary\nNC{args.nc} / NB{args.nb}; fixed H{args.hydrogen}; marker raw {onset:g} ps = grid {provenance["grid_onset_ps"]:g} ps')
    ...
    (p/'tautomer_summary_provenance.json').write_text(json.dumps({
        'script': str(Path(__file__)), 'source': provenance['source'], 'NC_id': args.nc, 'NB_id': args.nb,
        'fixed_H_id': args.hydrogen, 'raw_stop_ps': stop, 'raw_marker_ps': onset,
        'definition': 'LCOD = minimum-image r(NC,H) - minimum-image r(NB,H), angstrom; fixed atom IDs.',
        ...
        'plot_data': ['summary_tautomer_timeseries.csv', 'summary_fes_snapshot.csv', 'summary_pka.csv', 'summary_pka_window_samples.csv', 'summary_bpp_heavy_charges.csv', 'summary_bpp_competitors.csv'],
