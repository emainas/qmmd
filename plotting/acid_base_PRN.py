#!/usr/bin/env python3
"""PRN adaptation of acid_base_HIST: acid O4, H11, and one raw dihedral panel.

The inherited HIST water-wire/FES summary is retained. Batch screening and
snapshot-safe readers are supplied by prn_acid_base_analysis. No jobs are changed.
"""
...
def iter_xyz_frames(path: Path) -> Iterator[tuple[float | None, np.ndarray]]:
    """Read only complete XYZ frames from a fixed-size snapshot."""
    from prn_acid_base_analysis import xyz_frames
    yield from xyz_frames(path)
...
def shortest_oxygen_path(
    ...
    """Return the shortest O--O...O(defect) path in one frame.

    A direct O--O(defect) contact has zero bridging waters.  The returned
    ``oxygen_path`` excludes the acid oxygen but includes the defect oxygen.
    """
    if cutoff <= 0:
    ...
    natoms = len(coords)
    if not (1 <= acid_oxygen_id <= natoms and 1 <= defect_oxygen_id <= natoms):
    ...
    node_ids = [int(acid_oxygen_id)] + [x for x in oxygen_ids if x != acid_oxygen_id]
...
def _path_edge_distances(
    ...
    ids = [acid_oxygen_id, *oxygen_path]
    values: list[float] = []
...
def _hbond_shape_metrics(
    ...
    Path vectors are consistently directed from the selected acid oxygen toward
    ...
    """
    atom_ids = [acid_oxygen_id, *oxygen_path]
    vectors = np.asarray(
        [
...
def _hbond_orientation_ksi(
    ...
    defect-to-N edge and -1 when it is the acid oxygen-side atom.
    """
    defect_to_n_ids = list(reversed([acid_oxygen_id, *oxygen_path]))
    edge_by_nodes = {
        frozenset((edge.donor_id, edge.acceptor_id)): edge for edge in edges
    }
    signs: list[int] = []
    for hydronium_side, acid_oxygen_side in zip(
        defect_to_n_ids, defect_to_n_ids[1:]
    ):
        edge = edge_by_nodes.get(frozenset((hydronium_side, acid_oxygen_side)))
        if edge is None:
            raise ValueError(
        ...
        if edge.donor_id == hydronium_side:
            signs.append(1)
        elif edge.donor_id == acid_oxygen_side:
            signs.append(-1)
        else:
...
def defect_hbond_coordination(
    ...
    partners are the designated solute acid oxygen and all solvent oxygens.
    """
    natoms = len(coords)
    if not (1 <= defect_oxygen_id <= natoms):
        return -1
    partner_ids = sorted(
        ({int(acid_oxygen_id)} | {int(x) for x in solvent_oxygen_ids})
        - {int(defect_oxygen_id)}
    )
    hydrogen_ids_array = np.asarray(hydrogen_ids, dtype=int)
    ...
    for row_index, raw_defect_id in enumerate(defect_oxygen_ids):
        ...
        if hbond.connected:
            mean_angle, loopiness = _hbond_shape_metrics(
                coords, acid_oxygen_id, hbond.oxygen_path, box
            )
            orientation_signs, ksi = _hbond_orientation_ksi(
                acid_oxygen_id, hbond.oxygen_path, hbond_edges
            )
        ...
        yield FrameResult(
            ...
            oxygen_edge_distances=_path_edge_distances(
                coords, acid_oxygen_id, oxygen.oxygen_path, box
            ),
            hbond=hbond,
...
def augment_csv(
    ...
    extra_fields = [
        ...
        "hbond_orientation_signs_defect_to_acid_O",
        "hbond_ksi",
        "hbond_topology_defect_to_acid_O",
    ...
    for frame_number, (row, result) in enumerate(zip(rows, results)):
        ...
        hbond = result.hbond
        oxygen_full_path = [acid_oxygen_id, *oxygen.oxygen_path]
        ...
        row["hbond_orientation_signs_defect_to_acid_O"] = ";".join(
            f"{sign:+d}" for sign in result.hbond_orientation_signs
        )
        row["hbond_ksi"] = f"{result.hbond_ksi:.8f}"
        row["hbond_topology_defect_to_acid_O"] = result.hbond_topology
...
def load_pka_series(
    ...
    times, blocks = aligned_fes(fes_path)
    nblocks = len(blocks)
    if nblocks == 0:
...
def plot_wire_csv(
    ...
    dihedral_times: np.ndarray | None = None,
    dihedrals: np.ndarray | None = None,
    dihedral_ids: Sequence[int] = (5, 3, 4, 11),
    dihedral_center: float = 180.0,
    bond_dihedral_data: dict[str, np.ndarray] | None = None,
    figure_title: str = "PRN metadynamics",
    manual_diffusion: bool = False,
    competitor_data: np.ndarray | None = None,
    competitor_id: int = 5,
    competitor_max_bridging_waters: int = 3,
    deprotonation_start: float | None = None,
) -> None:
    """Plot wire connectivity and the aligned O--defect distance against time."""
    ...
    hbond_topology = np.asarray(
        [row.get("hbond_topology_defect_to_acid_O", "") for row in rows], dtype=object
    )
    ...
    acid_oxygen_label = distance_column.split("_Odefect_distance_A", 1)[0]
    ...
    has_dihedral = dihedral_times is not None and dihedrals is not None
    has_acquisition = (has_dihedral and bond_dihedral_data is not None
                       and f"O{dihedral_ids[2]}_acquired_hydrogen_id" in bond_dihedral_data)
    column_count = 3 + int(has_dihedral) + int(competitor_data is not None)
    ...
    event_legend = ("purple: sustained labeled-H loss; black: MANUAL diffusion; red: window end"
                    if manual_diffusion else "purple: sustained separated state; black: diffusion; red: window end")
    if manual_diffusion and deprotonation_start is None:
        event_legend = "no sustained labeled-H loss marker; black: MANUAL diffusion; red: window end"
    fig.suptitle(figure_title + " | " + event_legend, fontsize=13)
    ...
    if has_dihedral:
        ...
        for axis in axes[:, 3]:
            axis.remove()
        ax_dihedral = fig.add_subplot(grid[:2, 3] if has_acquisition else grid[:, 3])
        if has_acquisition:
            ax_acquired = fig.add_subplot(grid[2, 3])
    time_axes = [
        ax_distance,
        ax_hbond,
        ax_coordination,
    ...
    if has_dihedral:
        time_axes.append(ax_dihedral)
    if has_acquisition:
        time_axes.append(ax_acquired)
    if competitor_data is not None:
        competitor_axes = list(axes[:, -1])
        time_axes.extend(competitor_axes)
    ...
    if fes_path is None:
        ...
        active_axes = list(axes[:, :3].flat)
    if has_dihedral:
        active_axes.append(ax_dihedral)
    if has_acquisition:
        active_axes.append(ax_acquired)
        bd = bond_dihedral_data
        t = bd["time_ps"]
        after_loss = (t >= deprotonation_start) if deprotonation_start is not None else np.zeros(t.shape, dtype=bool)
        for oxygen, color in ((dihedral_ids[2], "#0072B2"), (dihedral_ids[0], "#D55E00")):
            ids = bd[f"O{oxygen}_acquired_hydrogen_id"]
            distance = bd[f"O{oxygen}_acquired_bond_distance_A"]
            observed = np.unique(ids[after_loss & np.isfinite(ids)]).astype(int)
            if not len(observed):
                ax_acquired.plot([], [], color=color, label=f"O{oxygen}: none assigned")
            for hydrogen in observed:
                visible = after_loss & (ids == hydrogen)
                values = np.where(visible, distance, np.nan)
                ax_acquired.plot(t, values, color=color, lw=1.2, label=f"O{oxygen}–H{hydrogen}")
                starts = np.flatnonzero(visible & ~np.r_[False, visible[:-1]])
                for start in starts:
                    ax_acquired.annotate(f"H{hydrogen}", (t[start], distance[start]),
                                        xytext=(3, 12), textcoords="offset points", fontsize=8, color=color)
        ax_acquired.axhline(float(bd["acquired_bond_cutoff_A"][0]), color=".5", ls="--", lw=.8)
        ax_acquired.set(title=f"New H after labeled-H loss (H{dihedral_ids[3]} excluded)",
                        ylabel="New O–H bond distance (Å)", xlabel=xlabel,
                        ylim=(.7, float(bd["acquired_bond_cutoff_A"][0])+.2))
        ax_acquired.legend(loc="lower left", fontsize=8)
        if deprotonation_start is None:
            ax_acquired.text(.04, .5, "No sustained labeled-H loss marker", transform=ax_acquired.transAxes, fontsize=8)
    if competitor_data is not None:
        active_axes.extend(competitor_axes)
        ax_charge, ax_wire, ax_all_h = competitor_axes
        ct, cq, cw, cs = competitor_data.T
        ax_charge.plot(ct, cq, color="#D55E00", linewidth=1.1)
        ax_charge.set_title(f"Competitor: oxygen {competitor_id}")
        ax_charge.set_ylabel(f"O{competitor_id} Mulliken charge (e)")
        states = np.arange(-1, competitor_max_bridging_waters + 1)
        cmap = ListedColormap(plt.get_cmap("turbo")(np.linspace(.04, .96, len(states))))
        norm = BoundaryNorm(np.arange(-1.5, competitor_max_bridging_waters + 1.5), cmap.N)
        finite = np.isfinite(cw)
        ax_wire.scatter(ct[finite], cw[finite], c=cw[finite], cmap=cmap, norm=norm,
                        s=14, edgecolors="none", rasterized=True)
        ax_wire.set_yticks(states)
        ax_wire.set_ylim(-1.5, competitor_max_bridging_waters + .5)
        ax_wire.set_ylabel(f"O{competitor_id}–defect bridging waters")
        window = finite & (ct > diffusive_start) & (ct <= probability_tmax)
        probability = float(np.mean(cw[window] >= 0)) if np.any(window) else np.nan
        ax_wire.set_title(f"P(conn) = {probability:.2f}; −1 disconnected")
        ax_all_h.plot(ct, cs, color="#009E73", linewidth=1.1)
        ax_all_h.set_ylabel(f"O{competitor_id} coordination with all H")
        ax_all_h.set_ylim(0, max(2.05, float(np.nanmax(cs)) * 1.05))
        ax_all_h.set_xlabel(xlabel)
        for axis in competitor_axes:
            axis.grid(axis="y", alpha=.2)
        ax_charge.tick_params(labelbottom=False)
        ax_wire.tick_params(labelbottom=False)

    if has_dihedral:
        from prn_anti_dih import wrapped_plot_series
        if bond_dihedral_data is not None:
            bd = bond_dihedral_data
            tx, phi = wrapped_plot_series(bd["time_ps"], bd["old_dihedral_deg"], center=dihedral_center)
            ax_dihedral.plot(tx, phi, color="#0072B2", linewidth=1.,
                             label="Old: " + "–".join(map(str, dihedral_ids)))
            new_ids = bd["new_hydrogen_id"]
            # Plot identities separately so a line never connects different protons.
            observed_ids = np.unique(new_ids[np.isfinite(new_ids)]).astype(int)
            for index, hydrogen_id in enumerate(observed_ids):
                values = np.where(new_ids == hydrogen_id, bd["new_dihedral_deg"], np.nan)
                tx, phi = wrapped_plot_series(bd["time_ps"], values, center=dihedral_center)
                ax_dihedral.plot(tx, phi, color="#D55E00", linewidth=1.,
                                 label=(f"New: {dihedral_ids[2]}–{dihedral_ids[1]}–{competitor_id}–Hnew"
                                        if index == 0 else None))
            if not observed_ids.size:
                ax_dihedral.plot([], [], color="#D55E00", label="New: no sustained O–H assignment")
            ax_dihedral.legend(loc="upper left", fontsize=9, framealpha=.9)
            ax_dihedral.set_title("Bond-resolved dihedrals")
            ax_dihedral.set_ylabel("O–C–O–H dihedral (degrees)")
        else:
            tx, phi = wrapped_plot_series(dihedral_times, dihedrals, center=dihedral_center)
            ax_dihedral.plot(tx, phi, color="#0072B2", linewidth=0.8)
            ax_dihedral.set_ylabel("dihedral " + "–".join(map(str, dihedral_ids)) + " (degrees)")
            ax_dihedral.set_title("PRN dihedral (unsmoothed)")
        ax_dihedral.set_ylim(dihedral_center - 180, dihedral_center + 180)
        ax_dihedral.set_yticks(np.arange(dihedral_center - 180, dihedral_center + 181, 60))
        ax_dihedral.set_xlabel(xlabel)
        if bond_dihedral_data is None and manual_diffusion and deprotonation_start is None:
            ax_dihedral.text(.04, .97, "Trace ends at MANUAL diffusion\nNo sustained proton-loss marker",
                             transform=ax_dihedral.transAxes, va="top", fontsize=10)
        if bond_dihedral_data is None and deprotonation_start is not None:
            ax_dihedral.text(
                0.04, 0.97, ("Trace ends before sustained\nlabeled-H coordination loss"
                             if manual_diffusion else "Trace ends before sustained\nseparated deprotonation"),
                transform=ax_dihedral.transAxes, va="top", fontsize=10,
            )
    ...
    ax_distance.legend(
        handles=[
            Line2D(
                ...
                linestyle="none",
                markersize=7,
                label=rf"{acid_oxygen_label}--defect",
            ),
            Line2D(
                [],
                [],
    ...
    ax_distance.set_ylabel(
        rf"{acid_oxygen_label}--O(defect$^+$) distance ($\AA$)"
    )
...
if __name__ == "__main__":
    from prn_acid_base_analysis import main
    main()
