"""Portable BPP run-9 tautomer movie; geometry only, no defect analysis."""
...
proc bpp_draw {name element op} {
    ...
    foreach pair [split [lindex $row 6] {;}] {
        ...
        if {$d == 30 || $d == 19} {graphics $mid color yellow}
    ...
    foreach id {30 19} label {NC NB} colour {blue orange} {
    ...
    # Fixed-H LCOD vectors, present at every frame: NC30 -> H31 and NB19 -> H31.
    ...
    graphics $mid sphere [lindex $xyz 30] radius .24 resolution 18
    ...
    graphics $mid text {-8 -9.4 0} "HC31 owner ID: [lindex $row 11] (0=shared/unassigned); closest water O[lindex $row 12]: [format %.2f [lindex $row 13]] A" size .75
    graphics $mid text {-8 -8 0} "[lindex $row 5] | elapsed [lindex $row 2] ps | raw [lindex $row 1] ps" size .9 thickness 1
    graphics $mid text {-8 -8.7 0} "NC-H31 [lindex $row 8] A | NB-H31 [lindex $row 9] A | LCOD [lindex $row 10] A" size .85 thickness 1
}
trace add variable ::vmd_frame($mid) write bpp_draw
animate goto 0
bpp_draw {} {} {}
display resetview
animate speed .08
...
puts "NC blue; NB orange; fixed H31 yellow; both LCOD vectors light green in all frames."
puts "Commands: tautomer_nc; tautomer_nb; animate forward; animate pause"
...
def main() -> None:
    ...
    parser.add_argument('--source', type=Path, default=Path('systems/BPP/solv_4.0/dftb/N1T64C1/run-9/meta-hic'))
    parser.add_argument('--topology', type=Path, default=Path('systems/BPP/init/bpp.mol2'))
    parser.add_argument('--out', type=Path, default=Path('reports/taut_bpp_meta_hic/run-9/vmd_tautomers'))
    ...
    parser.add_argument('--stop-raw-ps',type=float,default=32.25,help='Last included raw timestamp, ps')
    ...
    assert len(atoms) == 77 and atoms[29][1] == 'NC' and atoms[18][1] == 'NB' and atoms[30][1] == 'HC'
    ...
    while line := stream.readline():
        ...
        image = minimum_image(xyz-xyz[29], box)
        seen = {29}
        ...
        nc = [h+1 for d, h in bonds if d == 29]
        nb = [h+1 for d, h in bonds if d == 18]
        state = 'NC-H tautomer' if nc and not nb else 'NB-H tautomer' if nb and not nc else 'both sites protonated' if nc and nb else 'shared/unassigned H'
        frame_contacts = []
        for donor, h in bonds:
            if donor not in (29, 18):
                continue
            acceptor = 18 if donor == 29 else 29
        ...
        distances = np.linalg.norm(minimum_image(xyz[[29,18]]-xyz[30],box),axis=1)
        np.testing.assert_allclose(np.linalg.norm(image[[29,18]]-image[30],axis=1),distances,atol=1e-7)
        rows.append([len(rows), time, time-origin, ';'.join(map(str, nc)), ';'.join(map(str, nb)), state, ';'.join(f'{d+1}-{h+1}' for d, h in bonds), ';'.join(frame_contacts), *[f'{v:.6f}' for v in distances],f'{distances[0]-distances[1]:.6f}'])
        h_index = int(np.flatnonzero(hs==30)[0])
        assigned = next((d+1 for d,h in bonds if h==30),0)
        water_o = np.array([i for i,symbol in enumerate(symbols) if i>=77 and symbol=='O'])
        wd = np.linalg.norm(minimum_image(xyz[water_o]-xyz[30],box),axis=1)
        nearest_water = int(water_o[np.argmin(wd)])
        if wd.min() <= 3.5:
            np.testing.assert_allclose(np.linalg.norm(image[nearest_water]-image[30]),wd.min(),atol=1e-7)
    ...
    for name, header, data in (
        ('frames.csv', ['frame', 'raw_time_ps', 'elapsed_ps', 'NC_hydrogen_ids', 'NB_hydrogen_ids', 'state', 'geometric_H_bonds', 'hbond_H_acceptor_pairs', 'r_NC30_H31_A', 'r_NB19_H31_A', 'LCOD_A','HC31_assigned_owner_id','HC31_nearest_water_O_id','HC31_nearest_water_O_distance_A'], rows),
    ...
    (args.out/'README.txt').write_text(
        'Open with: vmd -e view_tautomers.vmd\nKeep all files together. Metadynamics from its first frame to raw 32.25 ps (grid 52.25 ps); no equil prefix.\n'
        'HC31 yellow; closest solvent oxygen highlighted red within 3.5 A; owner and water distance shown each frame.\n'
        'NC30 blue; NB19 orange; their currently bound H yellow; solute thick bonds.\n'
        'Both fixed LCOD vectors NC30 -> H31 and NB19 -> H31 are light-green arrows at EVERY frame.\n'
        ...
        'Commands: tautomer_nc / tautomer_nb jump to representative frames; animate forward plays.\n'
