"""Replot DPP's saved summary as FES/CV/estimator plus NC/ND charge/LCOD/coordination.
...
def refresh_full_run(p: Path, provenance: dict, nc: int, nd: int) -> float:
    ...
    for t, xyz in xyz_frames(source/'traject'):
        ...
        coord.append([t, *[rational_coordination(np.linalg.norm(minimum_image(xyz[hs]-xyz[i-1],box),axis=1)) for i in (nc,nd)]])
    ...
    np.savetxt(p/'summary_dpp_competitors.csv',coord,delimiter=',',header='time_ps,NC_all_H_coordination,ND_all_H_coordination',comments='')
    water_ids = [i+1 for i,s in enumerate(symbols) if i>=77 and s=='O']
    endpoints = {'NC':nc,'ND':nd,**{f'O{i}':i for i in water_ids}}
    qt, q = calculate_histidine_charges(source/'mulliken',endpoints,77,stop)
    lookup = {round(float(t),8):i for i,t in enumerate(qt)}
    charge_rows = [[t,*[q[label][lookup[round(float(t),8)]] if round(float(t),8) in lookup else np.nan for label in ('NC','ND')]] for t in times]
    np.savetxt(p/'summary_dpp_heavy_charges.csv',charge_rows,delimiter=',',header='time_ps,NC_charge,ND_charge',comments='')
...
def lcod_series(source: Path, ids: tuple[int, int, int], stop: float) -> np.ndarray:
    """Return time, NC-H, ND-H, and their difference (ps and angstrom)."""
    box = box_from_input(source/'dftb.inp')
    ...
    with (source/'traject').open() as handle:
        while line := handle.readline():
            ...
            if [fields[i][0] for i in indices] != ['N', 'N', 'H']:
                raise ValueError('Expected NC, ND, H atom elements')
...
def main() -> None:
    ...
    parser.add_argument('--report', type=Path, default=Path('reports/taut_dpp_meta_hic/run-5'))
    parser.add_argument('--nc', type=int, default=31)
    parser.add_argument('--nd', type=int, default=43)
    parser.add_argument('--hydrogen', type=int, default=32, help='Fixed, one-based hydrogen ID; no ownership switching')
    ...
    parser.add_argument('--grid-onset-ps',type=float,default=49.,help='Marker on plot_cv_grid clock, ps')
    ...
    if min(args.nc, args.nd, args.hydrogen) < 1 or len({args.nc, args.nd, args.hydrogen}) != 3:
    ...
    for atom_id, name in ((args.nc,'NC'),(args.nd,'ND'),(args.hydrogen,'HC')):
        if atoms[atom_id-1][1] != name:
            raise ValueError(f'Topology atom {atom_id} is not {name}')
    if len(atoms)!=77 or frozenset((args.nc,args.hydrogen)) not in bonds:
        raise ValueError('Expected 77 solute atoms and an NC-HC bond')
    ...
    provenance = dict(source=str(source),grid_onset_ps=args.grid_onset_ps,
                      ...
                      competitors={'NC':args.nc,'ND':args.nd},solute_atoms=77,
    ...
    if (args.nc, args.nd) != (provenance['competitors']['NC'], provenance['competitors']['ND']):
        parser.error('NC/ND IDs must agree with the saved charge/coordination products')
    onset = provenance['raw_onset_ps']
    stop = refresh_full_run(p,provenance,args.nc,args.nd)
    summary = load_csv(p/'summary_tautomer_timeseries.csv')
    charges = load_csv(p/'summary_dpp_heavy_charges.csv')
    coord = load_csv(p/'summary_dpp_competitors.csv')
    ...
    lcod = lcod_series(Path(provenance['source']), (args.nc, args.nd, args.hydrogen), stop)
    ...
    np.savetxt(p/'summary_lcod.csv', np.column_stack([summary['time_ps'], joined]), delimiter=',',
               header=f'time_ps,r_NC{args.nc}_H{args.hydrogen}_A,r_ND{args.nd}_H{args.hydrogen}_A,LCOD_A', comments='')
    ...
    ax_cv.set(ylabel='Biased NC–HC coordination s(t)', ylim=(0, 1))
    ...
    for i, (label, atom) in enumerate([('NC', args.nc), ('ND', args.nd)]):
    ...
    ax_q.set(yticks=[-.35, .75], yticklabels=[f'NC{args.nc}', f'ND{args.nd}'], ylim=(-1.05, 1.45), ylabel='Mulliken charge tiers (ND offset +1.1 e)')
    ax_lcod.plot(lcod[:, 0], lcod[:, 3], color='#6A3D9A', lw=1.2)
    ax_lcod.axhline(0, color='.5', lw=.8, ls=':')
    ax_lcod.set(ylabel=f'r(NC{args.nc},H{args.hydrogen}) − r(ND{args.nd},H{args.hydrogen}) (Å)', title='Fixed-H LCOD: negative NC-side / positive ND-side')
    ...
    fig.suptitle(f'DPP {p.name}: tautomerization summary\nNC{args.nc} / ND{args.nd}; fixed H{args.hydrogen}; marker raw {onset:g} ps = grid {provenance["grid_onset_ps"]:g} ps')
    ...
    (p/'tautomer_summary_provenance.json').write_text(json.dumps({
        'script': str(Path(__file__)), 'source': provenance['source'], 'NC_id': args.nc, 'ND_id': args.nd,
        'fixed_H_id': args.hydrogen, 'raw_stop_ps': stop, 'raw_marker_ps': onset,
        'definition': 'LCOD = minimum-image r(NC,H) - minimum-image r(ND,H), angstrom; fixed atom IDs.',
        ...
        'plot_data': ['summary_tautomer_timeseries.csv', 'summary_fes_snapshot.csv', 'summary_pka.csv', 'summary_pka_window_samples.csv', 'summary_dpp_heavy_charges.csv', 'summary_dpp_competitors.csv'],
