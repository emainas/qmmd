"""Portable APP run-5 tautomer movie; geometry only, no defect analysis."""
...
proc app_draw {name element op} {
    ...
    foreach pair [split [lindex $row 6] {;}] {
        ...
        if {$d == 18 || $d == 1} {graphics $mid color yellow}
    ...
    foreach id {18 1} label {NB NA} colour {blue orange} {
    ...
    # Fixed-H LCOD vectors, present at every frame: NB18 -> H19 and NA1 -> H19.
    ...
    graphics $mid sphere [lindex $xyz 18] radius .24 resolution 18
    # Highlight the water oxygen closest to the migrating HB, if within 3.5 A.
    ...
    graphics $mid text {-8 -9.4 0} "HB19 owner ID: [lindex $row 11] (0=shared/unassigned); closest water O[lindex $row 12]: [format %.2f [lindex $row 13]] A" size .75
    graphics $mid text {-8 -8 0} "[lindex $row 5] | elapsed [lindex $row 2] ps | raw [lindex $row 1] ps" size .9 thickness 1
    graphics $mid text {-8 -8.7 0} "NB-H19 [lindex $row 8] A | NA-H19 [lindex $row 9] A | LCOD [lindex $row 10] A" size .85 thickness 1
}
trace add variable ::vmd_frame($mid) write app_draw
animate goto 0
app_draw {} {} {}
display resetview
animate speed .08
proc tautomer_nb {} {global representative_nb; animate goto $representative_nb}
proc tautomer_na {} {global representative_na; animate goto $representative_na}
puts "NB blue; NA orange; fixed H19 yellow; both LCOD vectors light green in all frames."
puts "Commands: tautomer_nb; tautomer_na; animate forward; animate pause"
...
def main() -> None:
    ...
    parser.add_argument('--out', type=Path, default=Path('reports/taut_app_meta_hib/run-5/vmd_tautomers'))
    ...
    parser.add_argument('--stop-raw-ps',type=float,default=29.75,help='Last included raw timestamp, ps')
    ...
    assert len(atoms) == 77 and atoms[17][1] == 'NB' and atoms[0][1] == 'NA' and atoms[18][1] == 'HB'
    ...
    while line := stream.readline():
        ...
        image = minimum_image(xyz-xyz[17], box)
        seen = {17}
        ...
        nb = [h+1 for d, h in bonds if d == 17]
        na = [h+1 for d, h in bonds if d == 0]
        state = 'NB-H tautomer' if nb and not na else 'NA-H tautomer' if na and not nb else 'both sites protonated' if nb and na else 'shared/unassigned H'
        frame_contacts = []
        for donor, h in bonds:
            if donor not in (17, 0):
                continue
            acceptor = 0 if donor == 17 else 17
        ...
        distances = np.linalg.norm(minimum_image(xyz[[17,0]]-xyz[18],box),axis=1)
        np.testing.assert_allclose(np.linalg.norm(image[[17,0]]-image[18],axis=1),distances,atol=1e-7)
        rows.append([len(rows), time, time-origin, ';'.join(map(str, nb)), ';'.join(map(str, na)), state, ';'.join(f'{d+1}-{h+1}' for d, h in bonds), ';'.join(frame_contacts), *[f'{v:.6f}' for v in distances],f'{distances[0]-distances[1]:.6f}'])
        h_index = int(np.flatnonzero(hs==18)[0])
        assigned = next((d+1 for d,h in bonds if h==18),0)
        water_o = np.array([i for i,symbol in enumerate(symbols) if i>=77 and symbol=='O'])
        wd = np.linalg.norm(minimum_image(xyz[water_o]-xyz[18],box),axis=1)
        nearest_water = int(water_o[np.argmin(wd)])
        if wd.min() <= 3.5:
            np.testing.assert_allclose(np.linalg.norm(image[nearest_water]-image[18]),wd.min(),atol=1e-7)
    ...
    for name, header, data in (
        ('frames.csv', ['frame', 'raw_time_ps', 'elapsed_ps', 'NB_hydrogen_ids', 'NA_hydrogen_ids', 'state', 'geometric_H_bonds', 'hbond_H_acceptor_pairs', 'r_NB18_H19_A', 'r_NA1_H19_A', 'LCOD_A','HB19_assigned_owner_id','HB19_nearest_water_O_id','HB19_nearest_water_O_distance_A'], rows),
    ...
    (args.out/'README.txt').write_text(
        'Open with: vmd -e view_tautomers.vmd\nKeep all files together. Metadynamics from its first frame to raw 29.75 ps (grid 49.75 ps); no equil prefix.\n'
        'HB19 yellow; closest solvent oxygen highlighted red within 3.5 A; owner and water distance shown each frame.\n'
        'NB18 blue; NA1 orange; their currently bound H yellow; solute thick bonds.\n'
        'Both fixed LCOD vectors NB18 -> H19 and NA1 -> H19 are light-green arrows at EVERY frame.\n'
        ...
        'Commands: tautomer_nb / tautomer_na jump to representative frames; animate forward plays.\n'
