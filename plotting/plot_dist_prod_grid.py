#!/usr/bin/env python3
"""Plot prod-only distance time-series grids."""
...
def discover_runs(runs_path: Path, cv_dir: str, biaspot_name: str) -> List[Tuple[int, Path, Path]]:
    ...
    for p in sorted(runs_path.iterdir()):
        ...
        try:
            ...
            continue
        run_cv = p / cv_dir
        series = run_cv / "manual-cv" / "dist_prod.dat"
...
def main() -> None:
    p = argparse.ArgumentParser(description="Plot prod-only distance grids.")
    ...
    if not runs:
        raise SystemExit("No dist_prod.dat + biaspot found.")
