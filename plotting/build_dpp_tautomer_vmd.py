"""Portable DPP run-5 tautomer movie; geometry only, no defect analysis."""
...
proc dpp_draw {name element op} {
    ...
    foreach pair [split [lindex $row 6] {;}] {
        ...
        if {$d == 31 || $d == 43} {graphics $mid color yellow}
    ...
    foreach id {31 43} label {NC ND} colour {blue orange} {
    ...
    # Fixed-H LCOD vectors, present at every frame: NC31 -> H32 and ND43 -> H32.
    ...
    graphics $mid sphere [lindex $xyz 31] radius .24 resolution 18
    # Highlight the water oxygen closest to the migrating HC, if within 3.5 A.
    if {[lindex $row 13] <= 3.5} {
        set wid [lindex $row 12]
        graphics $mid color red
        set wp [lindex $xyz [expr {$wid-1}]]
        graphics $mid sphere $wp radius .30 resolution 16
        graphics $mid text [vecadd $wp {0 .4 0}] "water O$wid" size .8
    }
    graphics $mid color black
    graphics $mid text {-8 -9.4 0} "HC32 owner ID: [lindex $row 11] (0=shared/unassigned); closest water O[lindex $row 12]: [format %.2f [lindex $row 13]] A" size .75
    graphics $mid text {-8 -8 0} "[lindex $row 5] | elapsed [lindex $row 2] ps | raw [lindex $row 1] ps" size .9 thickness 1
    graphics $mid text {-8 -8.7 0} "NC-H32 [lindex $row 8] A | ND-H32 [lindex $row 9] A | LCOD [lindex $row 10] A" size .85 thickness 1
}
trace add variable ::vmd_frame($mid) write dpp_draw
animate goto 0
dpp_draw {} {} {}
display resetview
animate speed .08
proc tautomer_nc {} {global representative_nc; animate goto $representative_nc}
proc tautomer_nd {} {global representative_nd; animate goto $representative_nd}
puts "NC blue; ND orange; fixed H32 yellow; both LCOD vectors light green in all frames."
puts "Commands: tautomer_nc; tautomer_nd; animate forward; animate pause"
...
def main() -> None:
    ...
    parser.add_argument('--source', type=Path, default=Path('systems/DPP/solv_4.0/dftb/N1T64C1/run-5/meta-hic'))
    parser.add_argument('--topology', type=Path, default=Path('systems/DPP/init/dpp.mol2'))
    parser.add_argument('--out', type=Path, default=Path('reports/taut_dpp_meta_hic/run-5/vmd_tautomers'))
    ...
    parser.add_argument('--stop-raw-ps',type=float,default=30.75,help='Last included raw timestamp, ps')
    ...
    assert len(atoms) == 77 and atoms[30][1] == 'NC' and atoms[42][1] == 'ND' and atoms[31][1] == 'HC'
    ...
    while line := stream.readline():
        ...
        image = minimum_image(xyz-xyz[30], box)
        seen = {30}
        ...
        nd = [h+1 for d, h in bonds if d == 42]
        state = 'NC-H tautomer' if nc and not nd else 'ND-H tautomer' if nd and not nc else 'both sites protonated' if nc and nd else 'shared/unassigned H'
        frame_contacts = []
        for donor, h in bonds:
            if donor not in (30, 42):
                continue
            acceptor = 42 if donor == 30 else 30
        ...
        distances = np.linalg.norm(minimum_image(xyz[[30,42]]-xyz[31],box),axis=1)
        np.testing.assert_allclose(np.linalg.norm(image[[30,42]]-image[31],axis=1),distances,atol=1e-7)
        rows.append([len(rows), time, time-origin, ';'.join(map(str, nc)), ';'.join(map(str, nd)), state, ';'.join(f'{d+1}-{h+1}' for d, h in bonds), ';'.join(frame_contacts), *[f'{v:.6f}' for v in distances],f'{distances[0]-distances[1]:.6f}'])
        h_index = int(np.flatnonzero(hs==31)[0])
        assigned = next((d+1 for d,h in bonds if h==31),0)
        water_o = np.array([i for i,symbol in enumerate(symbols) if i>=77 and symbol=='O'])
        wd = np.linalg.norm(minimum_image(xyz[water_o]-xyz[31],box),axis=1)
        nearest_water = int(water_o[np.argmin(wd)])
        if wd.min() <= 3.5:
            np.testing.assert_allclose(np.linalg.norm(image[nearest_water]-image[31]),wd.min(),atol=1e-7)
        rows[-1].extend([assigned,nearest_water+1,float(wd.min())])
        frames.append(image)
    if not rows:
    ...
    for name, header, data in (
        ('frames.csv', ['frame', 'raw_time_ps', 'elapsed_ps', 'NC_hydrogen_ids', 'ND_hydrogen_ids', 'state', 'geometric_H_bonds', 'hbond_H_acceptor_pairs', 'r_NC31_H32_A', 'r_ND43_H32_A', 'LCOD_A','HC32_assigned_owner_id','HC32_nearest_water_O_id','HC32_nearest_water_O_distance_A'], rows),
    ...
    (args.out/'README.txt').write_text(
        'Open with: vmd -e view_tautomers.vmd\nKeep all files together. Metadynamics from its first frame to raw 30.75 ps (grid 50.75 ps); no equil prefix.\n'
        'HC32 yellow; closest solvent oxygen highlighted red within 3.5 A; owner and water distance shown each frame.\n'
        'NC31 blue; ND43 orange; their currently bound H yellow; solute thick bonds.\n'
        'Both fixed LCOD vectors NC31 -> H32 and ND43 -> H32 are light-green arrows at EVERY frame.\n'
        ...
        'Commands: tautomer_nc / tautomer_nd jump to representative frames; animate forward plays.\n'
