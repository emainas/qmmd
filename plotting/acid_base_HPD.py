#!/usr/bin/env python3
"""HPD adaptation of the HIST/PRD summary with integrated O7 competition.
...
def _circular_mean_deg(values: np.ndarray) -> float:
    """Return the circular mean of finite angles in degrees."""
...
def plot_wire_csv(
    ...
    hist_dihedral_times: np.ndarray | None = None,
    hist_dihedrals: dict[str, np.ndarray] | None = None,
    hist_charge_times: np.ndarray | None = None,
    hist_charges: dict[str, np.ndarray] | None = None,
    hist_competitor_wires: dict[str, np.ndarray] | None = None,
    hist_competitor_coordination: dict[str, np.ndarray] | None = None,
    hist_competitor_endpoint_ids: dict[str, int] | None = None,
    hist_competitor_max_bridging_waters: int = 3,
    hist_smoothing_window_ps: float = 0.10,
    oxygen_dihedral_data: dict[str, np.ndarray] | None = None,
    hist_conformer_bins: int = 36,
    hist_conformer_min_fraction: float = 0.01,
    ...
    has_hist_column = (
        hist_dihedral_times is not None
        and hist_dihedrals is not None
    )
    has_hist_competitor_column = (
        hist_charge_times is not None
        and hist_charges is not None
        and hist_competitor_wires is not None
        and hist_competitor_coordination is not None
        and hist_competitor_endpoint_ids is not None
    )
    column_count = (
        3 + int(has_hist_column) + int(has_hist_competitor_column) + int(oxygen_dihedral_data is not None)
    )
    ...
    if has_hist_column:
        ax_chi1, ax_chi2, ax_chi_map = axes[:, 3]
    if has_hist_competitor_column:
        ax_competitor_charge, ax_competitor_wire, ax_competitor_coord = axes[:, 3 + int(has_hist_column)]
    time_axes = [
        ax_distance,
        ax_hbond,
        ax_coordination,
    ...
    if has_hist_column:
        time_axes.extend([ax_chi1, ax_chi2])
    if has_hist_competitor_column:
        time_axes.extend(
    ...
    if oxygen_dihedral_data is not None:
        from hpd_proton_dihedral import plot_dihedral
        grid = axes[0,-1].get_subplotspec().get_gridspec()
        for old_axis in axes[:,-1]:
            if old_axis in active_axes:
                active_axes.remove(old_axis)
            old_axis.remove()
        ax_oh = fig.add_subplot(grid[:,-1], sharex=ax_distance)
        plot_dihedral(ax_oh, oxygen_dihedral_data)
        time_axes.append(ax_oh)
        active_axes.append(ax_oh)
    ...
    if has_hist_column:
        ...
        for axis, label in ((ax_chi1, "chi1"), (ax_chi2, "chi2")):
            values = np.asarray(hist_dihedrals[label], dtype=float)
            smoothed = _circular_running_average_deg(
                hist_dihedral_times, values, hist_smoothing_window_ps
            )
            finite = np.isfinite(hist_dihedral_times) & np.isfinite(smoothed)
            if not np.any(finite):
                continue
            reaction_start = (
                float(deprotonation_start)
                if deprotonation_start is not None
                else float(np.min(hist_dihedral_times[finite]))
            )
            diffusion_end = (
                float(probability_tmax)
                if probability_tmax is not None
                else float(np.max(hist_dihedral_times[finite]))
            )
            axis.axvspan(
                reaction_start,
                diffusive_start,
            ...
            reaction_mask = (
                finite
                & (hist_dihedral_times >= reaction_start)
                & (hist_dihedral_times < diffusive_start)
            )
            diffusion_mask = (
                finite
                & (hist_dihedral_times >= diffusive_start)
                & (hist_dihedral_times <= diffusion_end)
            )
            region_specs = (
                (
                    "reaction",
                    reaction_start,
                    diffusive_start,
                    _circular_mean_deg(values[reaction_mask]),
                ),
                (
                    "diffusion",
                    diffusive_start,
                    diffusion_end,
                    _circular_mean_deg(values[diffusion_mask]),
                ),
            )
            math_label = r"\chi_1" if label == "chi1" else r"\chi_2"
            for region_name, region_left, region_right, circular_mean in region_specs:
                if not np.isfinite(circular_mean) or region_right <= region_left:
                    continue
                region_width = region_right - region_left
                if region_name == "reaction":
                    label_x = region_right - 0.04 * region_width
                    horizontal_alignment = "right"
                else:
                    label_x = region_left + 0.04 * region_width
                    horizontal_alignment = "left"
                axis.text(
                    label_x,
                    0.06,
                    region_name + "\n" + rf"$\langle {math_label}\rangle_{{circ}}={circular_mean:.1f}^\circ$",
        ...
        for panel in (ax_chi1, ax_chi2):
            panel.set_ylim(-180.0, 180.0)
            panel.set_yticks([-180, -90, 0, 90, 180])
            panel.grid(axis="y", alpha=0.25)
        ax_chi1.set_ylabel(r"$\chi_1$ (deg)")
        ax_chi2.set_ylabel(r"$\chi_2$ (deg)")
        ax_chi2.set_xlabel(xlabel)

        chi1 = (np.asarray(hist_dihedrals["chi1"], dtype=float) + 180.0) % 360.0 - 180.0
        chi2 = (np.asarray(hist_dihedrals["chi2"], dtype=float) + 180.0) % 360.0 - 180.0
        map_window = (
            np.isfinite(hist_dihedral_times)
            & np.isfinite(chi1)
            & np.isfinite(chi2)
        )
        if probability_tmax is not None:
            map_window &= hist_dihedral_times <= probability_tmax
        finite_x = x[np.isfinite(x)]
        if finite_x.size:
            map_window &= hist_dihedral_times >= float(np.min(finite_x))
        histogram, chi1_edges, chi2_edges = np.histogram2d(
            chi1[map_window],
            chi2[map_window],
            bins=hist_conformer_bins,
            range=((-180.0, 180.0), (-180.0, 180.0)),
        )
        conformer_mesh = ax_chi_map.pcolormesh(
            chi1_edges,
            chi2_edges,
            histogram.T,
            cmap="RdBu",
            vmin=0.0,
            vmax=max(1.0, float(np.max(histogram))),
            shading="flat",
            rasterized=True,
        )
        conformer_colorbar = fig.colorbar(
            conformer_mesh, ax=ax_chi_map, pad=0.02
        )
        conformer_colorbar.set_label("frame count")
        ax_chi_map.axvline(-120.0, color="0.2", linestyle="--", linewidth=0.8)
        ax_chi_map.axvline(0.0, color="0.2", linestyle="--", linewidth=0.8)
        ax_chi_map.axvline(120.0, color="0.2", linestyle="--", linewidth=0.8)
        ax_chi_map.axhline(0.0, color="0.2", linestyle="--", linewidth=0.8)
        from hpd_competitor_analysis import classify_histidine_conformers

        conformer_labels = classify_histidine_conformers(chi1, chi2)
        map_count = int(np.count_nonzero(map_window))
        for label in ("tp", "tm", "gpp", "gpm", "gmp", "gmm"):
            selected = map_window & (conformer_labels == label)
            count = int(np.count_nonzero(selected))
            if not count or count / max(1, map_count) < hist_conformer_min_fraction:
                continue
            mean_chi1 = float(
                np.degrees(
                    np.arctan2(
                        np.mean(np.sin(np.radians(chi1[selected]))),
                        np.mean(np.cos(np.radians(chi1[selected]))),
                    )
                )
            )
            mean_chi2 = float(
                np.degrees(
                    np.arctan2(
                        np.mean(np.sin(np.radians(chi2[selected]))),
                        np.mean(np.cos(np.radians(chi2[selected]))),
                    )
                )
            )
            mean_chi1 = float(np.clip(mean_chi1, -165.0, 165.0))
            mean_chi2 = float(np.clip(mean_chi2, -165.0, 165.0))
            annotation = ax_chi_map.text(
                mean_chi1,
                mean_chi2,
                label,
                ha="center",
            ...
            annotation.set_path_effects(
                [patheffects.withStroke(linewidth=2.0, foreground="black")]
            )
        ax_chi_map.set_xlim(-180.0, 180.0)
        ax_chi_map.set_ylim(-180.0, 180.0)
        ax_chi_map.set_xticks([-180, -90, 0, 90, 180])
        ax_chi_map.set_yticks([-180, -90, 0, 90, 180])
        ax_chi_map.set_xlabel(r"$\chi_1$ (deg)")
        ax_chi_map.set_ylabel(r"$\chi_2$ (deg)")

    if has_hist_competitor_column:
        competitor_labels = tuple(hist_competitor_endpoint_ids)
        competitor_colors = {label: plt.get_cmap("tab10")(i) for i,label in enumerate(competitor_labels)}
        charge_band_height = 1.10
        for band_index, label in enumerate(competitor_labels):
            charge_values = np.asarray(hist_charges[label], dtype=float)
            finite = np.isfinite(hist_charge_times) & np.isfinite(charge_values)
            offset = band_index * charge_band_height
            ax_competitor_charge.plot(
                hist_charge_times[finite],
            ...
            if band_index:
                ax_competitor_charge.axhline(
                    offset - 0.90, color="0.82", linewidth=0.7, zorder=0
                )
        ax_competitor_charge.set_yticks(
            [index * charge_band_height - 0.35 for index in range(len(competitor_labels))],
            labels=[
                f"{label} ({hist_competitor_endpoint_ids[label]})"
                for label in competitor_labels
            ],
        )
        ax_competitor_charge.set_ylim(
            -1.05, (len(competitor_labels) - 1) * charge_band_height + 0.35
        )
        ax_competitor_charge.set_ylabel("HPD heavy-atom charge tier")

        competitor_wire_values = np.arange(
            -1, hist_competitor_max_bridging_waters + 1
        )
        ...
        competitor_wire_norm = BoundaryNorm(
            np.arange(-1.5, hist_competitor_max_bridging_waters + 1.5),
            competitor_wire_cmap.N,
        )
        band_height = hist_competitor_max_bridging_waters + 2.0
        wire_mappable = None
        for band_index, label in enumerate(competitor_labels):
            values = np.asarray(hist_competitor_wires[label], dtype=float)
            ...
            ax_competitor_wire.text(
                0.01,
                offset + hist_competitor_max_bridging_waters - 0.10,
        ...
        ax_competitor_wire.set_yticks(
            [
                index * band_height
                + 0.5 * (hist_competitor_max_bridging_waters - 1)
        ...
        ax_competitor_wire.set_ylim(
            ...
            + hist_competitor_max_bridging_waters
            + 0.5,
        )
        ax_competitor_wire.set_ylabel("HPD heavy-atom site")
        ...
        for band_index, label in enumerate(competitor_labels):
            values = np.asarray(hist_competitor_coordination[label], dtype=float)
            ax_competitor_coord.plot(x, values, label=label,
                color="steelblue" if label == "N1" else "darkorange", linewidth=1)
        ax_competitor_coord.set_ylim(bottom=0)
        ax_competitor_coord.set_ylabel("Coordination with all H")
        ax_competitor_coord.set_xlabel(xlabel)
        ax_competitor_coord.legend(frameon=False)
        ax_competitor_coord.grid(alpha=0.2)
    ...
    if has_hist_column:
        ax_chi1.tick_params(axis="x", which="both", labelbottom=False)
        ax_chi2.tick_params(axis="x", which="both", labelbottom=True)
    if has_hist_competitor_column:
...
def main() -> None:
    ...
    parser.add_argument(
        "--hist-competitor-max-bridging-waters",
        type=int,
        default=3,
        help=(
            "Maximum water bridges searched for HPD heavy-atom site paths "
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Histidine Amber topology; enables the conformer and N/O-site columns"
        ),
    )
    parser.add_argument(
        "--hist-dihedral-data-out",
        type=Path,
        default=None,
        help="Aligned histidine chi1/chi2 CSV (default: derived from --out)",
    )
    parser.add_argument(
        "--hist-competitor-data-out",
        type=Path,
        default=None,
        help="Aligned histidine N/O wire/coordination CSV (default: derived from --out)",
    )
    parser.add_argument(
        "--hist-charge-data-out",
        type=Path,
        default=None,
        help="Histidine N/O Mulliken-charge CSV (default: derived from --out)",
    )
    parser.add_argument(
        "--hist-smoothing-window-ps",
        type=float,
        default=0.10,
        help="Centered circular running-average window for histidine angles in ps",
    )
    parser.add_argument(
        "--hist-conformer-bins",
        type=int,
        default=36,
        help="Bins per angular dimension in the chi1--chi2 histogram (default: 36)",
    )
    parser.add_argument(
        "--hist-conformer-min-fraction",
        type=float,
        default=0.01,
        help=(
            "Minimum frame fraction required to annotate an occupied conformer "
            "basin (default: 0.01)"
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument('--oh-bond-cutoff', type=float, default=1.4, help='O-H ownership cutoff (Å)')
    parser.add_argument('--oh-ownership-margin', type=float, default=.15, help='Distance advantage over next heavy atom (Å)')
    parser.add_argument('--oh-persistence-ps', type=float, default=.05, help='Minimum same-H assignment duration (ps)')
    args = parser.parse_args()

    if args.hist_smoothing_window_ps <= 0.0:
        parser.error("--hist-smoothing-window-ps must be positive")
    if args.hist_competitor_max_bridging_waters < 0:
        parser.error("--hist-competitor-max-bridging-waters must be nonnegative")
    if args.hist_conformer_bins < 2:
        parser.error("--hist-conformer-bins must be at least 2")
    if not 0.0 <= args.hist_conformer_min_fraction <= 1.0:
        parser.error("--hist-conformer-min-fraction must be between 0 and 1")
    ...
    hist_dihedral_times: np.ndarray | None = None
    hist_dihedrals: dict[str, np.ndarray] | None = None
    hist_dihedral_out: Path | None = None
    hist_charge_times: np.ndarray | None = None
    hist_charges: dict[str, np.ndarray] | None = None
    hist_competitor_wires: dict[str, np.ndarray] | None = None
    hist_competitor_coordination: dict[str, np.ndarray] | None = None
    hist_competitor_out: Path | None = None
    hist_charge_out: Path | None = None
    hist_competitor_endpoints: dict[str, int] | None = None
    from hpd_competitor_analysis import (
        calculate_histidine_competitor_geometry, save_histidine_competitor_data,
    )
    symbols = read_xyz_symbols(args.traj)
    hist_competitor_endpoints = {"N1": 1, "O7": 7}
    if args.solute_atoms != 12 or symbols[0] != "N" or symbols[6] != "O" or symbols[7] != "H":
        raise ValueError("Expected HPD atom order: N1, competing O7, labeled H8; 12 solute atoms")
    with args.out.open() as handle:
        aligned_rows = list(csv.DictReader(handle))
    hist_dihedral_times = np.array([float(row["time_ps"]) for row in aligned_rows])
    aligned_defect_ids = np.array([float(row.get("defect_oxygen_id") or "nan") for row in aligned_rows])
    hist_competitor_wires, hist_competitor_coordination = calculate_histidine_competitor_geometry(
        args.traj, hist_dihedral_times, aligned_defect_ids, hist_competitor_endpoints,
        args.solute_atoms, box, max_bridging_waters=args.hist_competitor_max_bridging_waters,
        covalent_cutoff=args.covalent_cutoff, hydrogen_acceptor_cutoff=args.hydrogen_acceptor_cutoff,
        angle_cutoff=args.angle_cutoff)
    hist_competitor_out = args.out.with_name(f"{args.out.stem}_hpd_competitors.csv")
    save_histidine_competitor_data(hist_competitor_out,hist_dihedral_times,aligned_defect_ids,
                                  hist_competitor_wires,hist_competitor_coordination)
    ...
    if mulliken_path.exists():
        ...
        if (
            hist_competitor_endpoints is not None
            and hist_dihedral_times is not None
        ):
            from hpd_competitor_analysis import (
                calculate_histidine_charges,
                save_histidine_charge_data,
            )

            hist_charge_times, hist_charges = (
                calculate_histidine_charges(
                    mulliken_path,
                    hist_competitor_endpoints,
                    args.solute_atoms,
                    float(np.nanmax(hist_dihedral_times)),
                )
            )
            hist_charge_out = (
                args.hist_charge_data_out
                or args.out.with_name(f"{args.out.stem}_hpd_heavy_charges.csv")
            )
            save_histidine_charge_data(
                hist_charge_out,
                hist_charge_times,
    ...
    from hpd_proton_dihedral import oxygen_dihedral
    oh_data = oxygen_dihedral(args.traj, box, float(np.max(hist_dihedral_times)),
                             args.oh_bond_cutoff, args.oh_ownership_margin, args.oh_persistence_ps)
    np.savetxt(args.out.with_name('summary_O7H_dihedral.csv'),np.column_stack(list(oh_data.values())),
               delimiter=',',header=','.join(oh_data),comments='')
    plot_wire_csv(
        args.out,
        plot_out,
        oxygen_dihedral_data=oh_data,
        cutoff=args.cutoff,
        max_bridging_waters=args.max_bridging_waters,
        ...
        hist_dihedral_times=hist_dihedral_times,
        hist_dihedrals=hist_dihedrals,
        hist_charge_times=hist_charge_times,
        hist_charges=hist_charges,
        hist_competitor_wires=hist_competitor_wires,
        hist_competitor_coordination=hist_competitor_coordination,
        hist_competitor_endpoint_ids=hist_competitor_endpoints,
        hist_competitor_max_bridging_waters=(
            args.hist_competitor_max_bridging_waters
        ),
        hist_smoothing_window_ps=args.hist_smoothing_window_ps,
        hist_conformer_bins=args.hist_conformer_bins,
        hist_conformer_min_fraction=args.hist_conformer_min_fraction,
    ...
    if hist_dihedral_out is not None:
        print(f"Wrote {hist_dihedral_out}")
    if hist_competitor_out is not None:
        print(f"Wrote {hist_competitor_out}")
    if hist_charge_out is not None:
        print(f"Wrote {hist_charge_out}")
