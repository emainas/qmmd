#!/usr/bin/env python3
"""Plot 2D CV time-series grids (CV1 red, CV2 blue) from biaspot files."""
...
def parse_biaspot_2d(path: Path) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    times: List[float] = []
    cv1: List[float] = []
    cv2: List[float] = []
    with path.open("r", encoding="utf-8") as f:
        while True:
            ...
            if not m:
                continue
            t = float(m.group(1)) / 1000.0  # fsec -> ps
            times.append(t)
            # read forward for two Coordinate lines
            cvals = []
            while len(cvals) < 2:
                line = f.readline()
                if not line:
                ...
                if m2:
                    cvals.append(float(m2.group(1)))
            if len(cvals) >= 2:
                cv1.append(cvals[0])
                cv2.append(cvals[1])
    n = min(len(times), len(cv1), len(cv2))
    if n == 0:
        return np.array([]), np.array([]), np.array([])
    t = np.array(times[:n])
    return t, np.array(cv1[:n]), np.array(cv2[:n])
...
def main() -> None:
    p = argparse.ArgumentParser(description="Plot 2D CV grids from biaspot files.")
    ...
    fig, axes = plt.subplots(rows, cols, figsize=(3.6 * cols, 2.8 * rows), dpi=220, sharex=False, sharey=False)
    axes_flat = axes.flatten() if isinstance(axes, np.ndarray) else [axes]

    for i, (run_id, biaspot) in enumerate(runs):
        ax = axes_flat[i]
        t, y1, y2 = parse_biaspot_2d(biaspot)
        if y1.size == 0:
            ax.set_axis_off()
            continue
        t0 = float(t[0])
        t_rel = t - t0
        ax.plot(t_rel, y1, color="red", lw=1.4, label="CV1")
        ax.plot(t_rel, y2, color="blue", lw=1.4, label="CV2")
    ...
    for r in range(rows):
        if cols > 1:
            axes[r, 0].set_ylabel("CV value", fontsize=10)
        else:
            axes.set_ylabel("CV value", fontsize=10)
    for c in range(cols):
        if rows > 1:
    ...
    out = args.out
    if out is None:
        out = Path("reports") / f"{system}_{args.cv_dir}_cv2d_{n}runs.png"
